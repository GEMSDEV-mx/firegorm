...
func (r *HookRegistry) RunHooks(ctx context.Context, collection string, ht HookType, data interface{}) error {
	...
	// Copy the callbacks and release the lock before invoking user code. Hooks may
	// register or toggle other hooks without deadlocking the registry.
	fns := append([]HookFunc(nil), r.hooks[collection][ht]...)
	r.mu.RUnlock()
	for _, fn := range fns {
